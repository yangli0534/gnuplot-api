...
class MainWindow(QtWidgets.QMainWindow):
    ...
    def __init__(self, *args, **kwargs):
        ...
        self.ul_freq_center = self.RuCalib.RU.UL_CENT_FREQ
        offs = -self.RuCalib.Station.get_tx_IL(freq)
        self.stimuli = -50
        self.rx_cable_IL = self.RuCalib.Station.get_rx_IL(self.ul_freq_center)
    ...
    def run(self):
        ...
        if (mode == 'Tx' and branch in ['A', 'B', 'C', 'D']):
            ...
            self.RuCalib.logger.critical(
                f'The calc DAC value in main of final of branch  {branch} is  {int(bias[0], 16)}')
            self.RuCalib.logger.critical(
                f'The calc DAC value in peak of final of branch  {branch} is  {int(bias[1], 16)}')
            self.RuCalib.logger.critical(
                f'The calc DAC value in main of driver of branch {branch} is {int(bias[2], 16)}')
            self.RuCalib.logger.critical(
                f'The calc DAC value in peak of driver of branch  {branch} is {int(bias[3], 16)}')
            ...
            temp = self.RuCalib.RU.read_temp_pa(branch)
            self.RuCalib.save_bias_db(branch,temp, bias)
            ...
            self.RuCalib.RU.pa_bias_read_curr_preset()
            ...
            if self.RuCalib.tx_carrier_setup(branch, cbw=20, freq=self.RuCalib.RU.DL_CENT_FREQ):
                (tx_alg_dsa_gain_set, tx_dpd_post_vca_gain_set) = self.RuCalib.tx_power_adjustment(branch, target=40,
                                                                                                   error=0.2)
                ...
                self.RuCalib.logger.info(f'Tx Calibration for branch {branch} finished')

                self.RuCalib.logger.info(f'Test tor gain accuracy for different temp')
                (temp_list, arp_pm_list, tor_pm_list) = self.RuCalib.test_tor_gain_temp(branch)
                self.RuCalib.logger.critical(f'ARP Power is {arp_pm_list}')
                self.RuCalib.logger.critical(f'Tor power is {tor_pm_list}')
                self.RuCalib.logger.critical(f'PA temp is {temp_list}')
                self.RuCalib.logger.info(f'Test tor gain accuracy for different temp finished')
